...
5. **Be careful with `drizzle-kit generate`.** The drizzle schema in `apps/api/drizzle/schema/workflow.ts` is currently missing five columns that `0049_workflow_custody.sql` added to the live database — `workflow_record_state.assignee_user_id`, `.assignee_role`, `.step_entered_at`, `.due_at`, and `workflow_transitions.handoff`. Grepping `workflow.ts` for any of those names returns nothing, while `src/modules/workflow/workflow.service.ts` reads and writes them extensively (lines 159, 177-178, 199-201, 221, 271, 348-352, 455-467) — all through raw SQL. A `db:generate` today would emit `DROP COLUMN` for all five. Fix the `.ts` before generating. `0044_snapshot_sync.sql`'s header describes the same class of problem and how it was handled last time.
