...
`meta/` also holds `NNNN_snapshot.json` files — drizzle's model of the schema at each point, used as the base for the next `generate` diff. Snapshots exist only for `0000`–`0029`, `0035`, `0044`, `0047` and `0067`. The gaps are the hand-written migrations, which drizzle never saw.

When the snapshot chain falls too far behind, the next `generate` emits a migration that **cannot run**: bare `CREATE TYPE` and bare `ADD COLUMN` statements with no `IF NOT EXISTS`, against objects that already exist. Three migrations exist purely to fix this:

- **`0035_snapshot_sync.sql`**, **`0044_snapshot_sync.sql`** and **`0067_snapshot_sync.sql`** are intentional no-ops (`SELECT 1;`). Their deliverable is the refreshed `meta/*.json` snapshot, not DDL.
...
`0067` is the same discipline applied to a worse case, and it is worth reading for the two failure modes only a byte-level comparison finds. It was drift in BOTH directions: the chain had stalled at `0047` while `0048`–`0066` were hand-written (23 bare `ADD COLUMN`s), **and** seventeen columns existed in the database and in no schema file at all, so drizzle could not see them in either direction. Its header records the verification statement by statement — 1 `CREATE TABLE` whose 13 columns match exactly, 23 `ADD COLUMN`s each present with the same type, nullability and default, 4 foreign keys present under exactly those names, and 10 `CREATE INDEX`es each rebuilt under a probe name inside a rolled-back transaction and compared with `pg_get_indexdef`. That last step is what caught `.desc()` (drizzle: `DESC NULLS LAST`) against a hand-written bare `DESC` (Postgres: `DESC NULLS FIRST`) — the same index, a different definition, and exactly what a later `generate` would offer to "fix" by rebuilding it. It also caught three foreign keys named `…_fkey` by hand-written SQL where an inline `.references()` makes drizzle say `…_users_id_fk`: drizzle's `EXCEPTION WHEN duplicate_object` does not forgive a *different* name, so the mismatch would have added a second, redundant foreign key rather than being skipped.

**Practical consequence:** if you hand-write a migration, either write a snapshot-sync no-op afterwards or expect the next `generate` to produce garbage. `guard-check.sh` asserts the three-way agreement (schema `.ts` ↔ newest snapshot ↔ live database) for the workflow tables, so that particular stall now fails the suite rather than waiting to be discovered.
...
| **0064** | `workflow_webhook_outbox.sql` | `workflow_webhook_outbox` — pending outbound deliveries; `workflow_webhook_secrets` — one HMAC key per workspace per environment, generated by the column default | The `webhook` action writes an outbox row INSIDE the business transaction and sends nothing, so a rollback takes the delivery with it and a receiver is only ever told about a move that really committed — the ordering objection answered by construction rather than by care. A separate dispatcher sends what committed, with backoff, an attempt ceiling and a `dead` state, claiming rows with `for update skip locked` so two pm2 instances divide the queue instead of duplicating it. The signing secret has no API that writes it and no default a human chooses: 64 hex characters from the same CSPRNG that mints every primary key, because a key somebody types is a key somebody can guess. A `url like 'https://%'` CHECK is the floor under `webhook-url.ts`, which is the real SSRF guard |
...
| **0067** | `snapshot_sync.sql` | **Intentional no-op** — `SELECT 1;`. The deliverable is `meta/0067_snapshot.json`, plus the schema-file repair that had to happen first | Drift in both directions, accumulated over nineteen hand-written migrations. The snapshot had stalled at `0047`, so `generate` emitted 23 bare `ADD COLUMN`s for columns that already existed; and seventeen columns existed in the database and in NO schema file, so drizzle could not see them at all — no types, unknown to `push` and `db:studio`, and any future migration for those tables would have been written against a table seventeen columns out of date. Every statement the diff wanted was verified against the live database before the file was emptied (see §8, *The snapshot trap*, for the two failure modes only the byte-level index comparison finds). Zero `DROP`s, zero `ALTER COLUMN`. `guard-check.sh` now asserts the three-way agreement so the next stall fails the suite |
...
**1. ~~`workflow.ts` is missing all five columns that `0049` added.~~ CLOSED by `0067_snapshot_sync`.** It was seventeen columns rather than five once `status_logs` and `approval_requests` were counted, and the consequence was not the one recorded here: because those columns were absent from *both* the `.ts` and the snapshot, `generate` could not emit `DROP COLUMN` for them — it simply could not see them. The statements it did emit were 23 bare `ADD COLUMN`s for columns that already existed, which dies on the first one. All seventeen are declared now, `meta/0067_snapshot.json` is the refreshed base, and `corepack pnpm db:generate` prints "No schema changes, nothing to migrate". `guard-check.sh` asserts the three-way agreement (schema `.ts` ↔ newest snapshot ↔ live database) over the workflow tables, so this cannot re-open silently.
