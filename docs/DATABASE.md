...
Seven tables now. The original four (`workflow_flows`, `workflow_steps`, `workflow_transitions`,
`workflow_record_state`) plus `workflow_exceptions` (0054), `workflow_auto_fired` (0055) and
`workflow_action_runs` (0056). `drizzle/schema/workflow.ts` is 281 lines (~14 KB), most of it
